define(["jquery"],function($){
    ...
    function Form_Authentication(){
        ...
        $(".password_box1b").find("input").blur(function(){
            if(!/^[0-9a-zA-Z]{8,16}$/.test(this.value)){
    ...
    function data_interaction(){
        $("#submit").find("button").click(function(){
            $.ajax({
                ...
                data:{
                    username: $(".Account_box1b input").val(),
                    password: $(".password_box1b input").val(),
                    repassword:$(".password_box2b input").val(),
                ...
                success:function(result){
                     ...
                     // alert(str)
                     if(obj.code){
                         $("#warning").css("color","red").css("background","pink");
                         $("#warning").html(obj.message);
             
                     }else{
                         $("#warning").css("color","green").css("background","#ffffff")
                         $("#warning").html(obj.message);
                         setTimeout(function(){
